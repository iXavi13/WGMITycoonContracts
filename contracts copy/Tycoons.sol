...
contract WGMITycoons is ERC1155, ERC1155Supply, ERC1155Burnable, AccessControl {
    ...
    function mintTycoons(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            uint256 id = ids[i];
            uint256 amount = mintAmounts[i];
            require(amount > 0, "Incorrect mint amount");
            require(id > 0, "Incorrect tycoon value");
            require(totalSupply(id) + amount < maxTycoonSupply[id] + 1, "Max supply reached");
            
            _mintTycoons(id, amount);
        }
    }
    ...
    function _mintTycoons(uint256 id, uint256 mintAmount) internal {
        ...
        if (id != 1)
            require(balanceOf(msg.sender, id - 1) + 1 > burnAmount * mintAmount, "Burn balance insufficient");
        
        moonz.burnFrom(msg.sender, cost * 1 ether);

        if (id != 1)
            burn(msg.sender, id - 1, burnAmount * mintAmount);
