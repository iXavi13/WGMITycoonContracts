...
contract TycoonConfig is AccessControl {
    ...
    function setInterfaces(IMoonz IMoonz_, IWGMITycoon IWGMITycoon_) external onlyRole(GAME_ADMIN) {
        game.setInterfaces(IMoonz_, IWGMITycoon_);
    }
    ...
    function setMultiplierMaxLevels(
        ...
        require(ids.length == maxLevel_.length, "Incorrect array lengths");
        game.setMultiplierMaxLevels(ids, maxLevel_);
    }
    ...
    function setCapacityMaxLevels(
        ...
        require(ids.length == maxLevel_.length, "Incorrect array lengths");
        game.setCapacityMaxLevels(ids, maxLevel_);
    }
